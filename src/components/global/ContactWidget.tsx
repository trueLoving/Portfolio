  ...
  return (
    <div className="fixed inset-0 z-[95] flex items-end md:items-center justify-end md:justify-center p-4">
      ...
      <div
        ...
        className="relative w-full md:max-w-lg bg-gray-900/95 text-white rounded-2xl shadow-2xl p-6 border border-white/10"
      >
        ...
        {success ? (
          <div className="mt-6 bg-green-600/10 border border-green-500/30 text-green-200 rounded-lg p-4">
            <p>
              {t('contact.success')} <span className="font-medium">{email}</span>
              {locale === 'zh-CN' ? t('contact.successSuffix') : '.'}
            </p>
          </div>
        ) : (
          <form className="mt-6 space-y-4" onSubmit={send}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-lg bg-white/5 border border-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-white/40"
            ...
            <div>
              ...
              <textarea
                ...
                rows={5}
                className="mt-1 w-full rounded-lg bg-white/5 border border-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-white/40 resize-y"
                placeholder={t('contact.message')}
                disabled={disabled}
              />
            </div>
            {error && (
              <div className="text-red-300 text-sm bg-red-500/10 border border-red-500/30 rounded-md p-2">
                {error}
              </div>
            )}
